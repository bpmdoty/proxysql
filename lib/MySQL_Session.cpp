...
*/
	...
	// handle case #1797
	// handle case #2564 - do not intercept "SELECT CONNECTION_ID()" in COM_STMT_PREPARE
	if ((pkt->size==SELECT_CONNECTION_ID_LEN+5 && (unsigned char)(pkt->ptr+4)!=0x16 && strncasecmp((char *)SELECT_CONNECTION_ID,(char *)pkt->ptr+5,pkt->size-5)==0)) {
		char buf[32];
		char buf2[32];
